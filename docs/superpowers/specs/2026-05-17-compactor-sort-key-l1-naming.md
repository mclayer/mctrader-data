...
pre_lookup_evidence:
  ...
  - "l3.py:74 sorted(rglob) — verified-via: git show origin/main:src/mctrader_data/compactor/l3.py path 에 hour=NN 가 part- 앞 = incidentally safe"
...
| 안전 | L3 `compact_day` path 에 `hour={hour_utc:02d}` zero-padded 가 `part-` 앞 + L2 `node=MERGED` 고정 = hour 당 1 L2 → byte-sort = hour-sort = incidentally safe. 단 hour 당 다중 L2 발생 시 regression | l3.py:74 + l2.py:79 out_dir node=MERGED |
...
- `compact_day` local ([l3.py:74](src/mctrader_data/compactor/l3.py#L74)) + `_compact_day_nas` 동일 `_extract_min_ts` key 적용.
...
planned_files:
  ...
  - path: src/mctrader_data/compactor/l3.py
    change: compact_day local + _compact_day_nas defensive sort key 동형 적용 (lines 74, 165)
